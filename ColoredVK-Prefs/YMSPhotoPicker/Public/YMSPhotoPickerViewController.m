...
static const CGFloat YMSPhotoFetchScaleResizingRatio = 1.0;
...
@property (nonatomic, readonly) BOOL canAddPhoto;
...
//    PHAsset *asset = fetchResult[indexPath.item-1];
    ...
    CGFloat scale = screenScale * YMSPhotoFetchScaleResizingRatio;
    // custom
...
//    else 
    if (!self.allowsMultipleSelection) {
        if (!self.shouldReturnImageForSingleSelection) {
            PHFetchResult *fetchResult = self.currentCollectionItem[@"assets"];
                //            PHAsset *asset = fetchResult[indexPath.item-1];
            PHAsset *asset = fetchResult[indexPath.item];
...
//        }
    ...
    albumPickerViewController.modalPresentationStyle = UIModalPresentationOverCurrentContext;
...
- (void)photoLibraryDidChange:(PHChange *)changeInstance {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (index != NSNotFound) {
            ...
            updatedCollectionItems[index] = self.currentCollectionItem;
