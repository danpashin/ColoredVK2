...
- (UIImage *) resizedImageByMagick: (NSString *) spec
{
    ...
    if([spec hasSuffix:@"!"]) {
        ...
        NSUInteger width = labs([widthAndHeight[0] integerValue]);
        NSUInteger height = labs([widthAndHeight[1] integerValue]);
    ...
    if([spec hasSuffix:@"^"]) {
        NSString *specWithoutSuffix = [spec substringToIndex: spec.length - 1];
        NSArray *widthAndHeight = [specWithoutSuffix componentsSeparatedByString: @"x"];
        return [self resizedImageWithMinimumSize: CGSizeMake (labs([widthAndHeight[0] integerValue]),
                                                              labs([widthAndHeight[1] integerValue]))];
    }
    ...
    if ([widthAndHeight[0] isEqualToString: @""]) {
        return [self resizedImageByHeight: labs([widthAndHeight[1] integerValue])];
    }
    return [self resizedImageWithMaximumSize: CGSizeMake (labs([widthAndHeight[0] integerValue]),
                                                          labs([widthAndHeight[1] integerValue]))];
}
